...
class BSRoformer9a(Fourier):
    def __init__(
        ...
        rotary_emb = RotaryEmbedding(dim=self.head_dim, freqs_for='pixel', max_freq=104)
        rotary_t1 = rotary_emb.get_axial_freqs(104)

        rotary_emb = RotaryEmbedding(dim=self.head_dim, freqs_for='pixel', max_freq=f1)
        rotary_f1 = rotary_emb.get_axial_freqs(f1)

        rotary_emb = RotaryEmbedding(dim=self.head_dim, freqs_for='pixel', max_freq=64)
        rotary_tf = rotary_emb.get_axial_freqs(64)
        ...
        for _ in range(self.depth):
            self.transformers.append(
                TransformerBlock(dim=self.dim, n_heads=self.n_heads, rotary_t=rotary_t1, rotary_f=rotary_f1, rotary_tf=rotary_tf)
                )
    ...
    def process_image(self, x):
        ...
        ds_ratio = self.t2 * 8
        pad_len = int(np.ceil(T / ds_ratio)) * ds_ratio - T
...
class Attention(nn.Module):
    ...
    def __init__(self, dim: int, n_heads: int, rotary_t: torch.Tensor, rotary_f, rotary_tf):
        super().__init__()
        ...
        self.rotary_tf = rotary_tf

        self.flash = hasattr(torch.nn.functional, 'scaled_dot_product_attention')
        assert self.flash, "Must have flash attention."
        
        self.c_attn = nn.Linear(dim, 7 * dim, bias=False)  # 3 indicates qkv outputs.
    ...
    def forward(self, x):
        ...
        q_t, q_f, q_tf, k_t, k_f, k_tf, v = rearrange(self.c_attn(x), 'b t f (r h d) -> r b h t f d', r=7, h=self.n_heads)
        ...
        ##
        q_tf = rearrange(q_tf, 'b h (t3 t4) (f3 f4) d -> (b t3 f3) h (t4 f4) d', t4=8, f4=8)
        k_tf = rearrange(k_tf, 'b h (t3 t4) (f3 f4) d -> (b t3 f3) h (t4 f4) d', t4=8, f4=8)
        v = rearrange(v, '(b t3 t4) h (f3 f4) d -> (b t3 f3) h (t4 f4) d', b=B, t4=8, f4=8)
    
        q_tf = apply_rotary_emb(self.rotary_tf.to(device), q_tf)
        k_tf = apply_rotary_emb(self.rotary_tf.to(device), k_tf)

        v = torch.nn.functional.scaled_dot_product_attention(q_tf, k_tf, v, attn_mask=None, dropout_p=0, is_causal=False)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim: int, n_heads: int, rotary_t: torch.Tensor, rotary_f, rotary_tf):
        ...
        self.att = Attention(dim=dim, n_heads=n_heads, rotary_t=rotary_t, rotary_f=rotary_f, rotary_tf=rotary_tf)
