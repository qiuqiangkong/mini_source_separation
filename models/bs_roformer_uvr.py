...
class BSRoformer(Module):
    ...
    def forward(
        ...
        device = mixure.device

        if mixure.ndim == 2:
            mixure = rearrange(mixure, 'b t -> b 1 t')

        channels = mixure.shape[1]
        ...
        mixure, batch_audio_channel_packed_shape = pack_one(mixure, '* t')

        stft_window = self.stft_window_fn(device = device)

        stft_repr = torch.stft(mixure, **self.stft_kwargs, window = stft_window, return_complex = True)
...
if __name__ == "__main__":
    ...
    model_config_path = '/home/zhiyuew/my_bs_roformer/model_bs_roformer_ep_317_sdr_12.9755.yaml'
    with open(model_config_path, 'r') as f:
        config = yaml.safe_load(f)['model']
        
    bs_roformer = BSRoformer(**config)
    
    bs_roformer.load_state_dict(torch.load('/home/zhiyuew/my_bs_roformer/model_bs_roformer_ep_317_sdr_12.9755.ckpt', ))

    audio_random = torch.randn(1, 2, int(44100 * 1.24))
    output = bs_roformer(audio_random)
    print(output.shape)