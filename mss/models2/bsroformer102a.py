...
from mss.models2.attention102a import Block
from mss.models.rope import RoPE
from mss.utils import fast_sdr


class BSRoformer102a(nn.Module):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
        ...
        **kwargs
        ...
        # Transformer blocks
        has_attns1 = [False, False, True, False, False, False]
        has_attns2 = [False, False, False, False, True, False]
        self.t_blocks = nn.ModuleList(Block(dim, n_heads, has_attns1[i]) for i in range(n_layers))
        self.k_blocks = nn.ModuleList(Block(dim, n_heads, has_attns2[i]) for i in range(n_layers))
