...
from mss.models2.dsp3.subband_fast_overlap import SubbandFilterOverlap2
from mss.models.attention import Block
from mss.models.rope import RoPE
from mss.utils import fast_sdr


class BSRoformer111b(nn.Module):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
        ...
        **kwargs
        ...
        # Subband filter
        banks = erb_linear_banks_triangle(sr=sample_rate, n_bands=n_bands, max_half_bandwidth=max_half_bandwidth)
        for i in range(1, len(banks) - 1):
            banks[i] = [banks[i][0], banks[i][2]]
        self.sb_filter = SubbandFilterOverlap2(sample_rate, banks, factor, chunk_size=chunk_size)
