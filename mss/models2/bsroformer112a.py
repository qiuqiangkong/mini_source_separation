...
from mss.models2.dsp3.lowband_fractional_stft import LowbandFractionalSTFT
from mss.models.attention import Block
from mss.models.rope import RoPE
from mss.utils import fast_sdr


class BSRoformer112a(nn.Module):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
        ...
        **kwargs
        ...
        self.stft = LowbandFractionalSTFT(subband_sr, half_bandwidths, self.n_fft, self.hop_length, 0.3)
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        subband_length = x.shape[-1]

        x = rearrange(x, 'b c k l -> (b c) k l')
        x = self.stft.analysis(x)
        complex_sp = rearrange(x, '(b c) k t f -> b c k t f', b=B)
        ...
        x = self.stft.synthesis(x, subband_length)  # (b, c, k, t, f)
    ...
    def check_sdr(self, audio, complex_sp) -> None:
        B = audio.shape[0]
        x = rearrange(complex_sp, 'b c k t f -> (b c) k t f')
        x = self.stft.synthesis(x, 800)  # (b, c, k, t, f)
        x = rearrange(x, '(b c) k l -> b c k l', b=B)

        y = self.sb_filter.synthesis(x)
