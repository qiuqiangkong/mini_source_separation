...
class BSRoformer100a(nn.Module):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
        ...
        **kwargs
        ...
        self.bandsplit = BandSplit(
            ...
            out_channels=self.band_dim
        )
        
        # Patch
        in_channels = self.band_dim * 2
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        # Subband Analysis
        complex_sp = self.stft(audio)  # (b, c, k, t, f)
        ...
        T = x.shape[2]
        x = self.pad_tensor(x, self.patch_size_t)  # x: (b, d, t, f)
        x = self.patch(x)
