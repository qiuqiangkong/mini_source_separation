...
class SelfAttention(nn.Module):
    def __init__(self, dim, num_heads) -> None:
        ...
        kernel_size = 4
        self.qkv_linear = nn.Conv1d(dim, 3 * dim, kernel_size=kernel_size, stride=kernel_size)
        self.norm_q = RMSNorm(dim)
        self.norm_k = RMSNorm(dim)

        # self.proj = nn.Linear(dim, dim)
        self.proj = nn.ConvTranspose1d(dim, dim, kernel_size=kernel_size, stride=kernel_size)
    ...
    def forward(
        ...
        q, k, v = rearrange(self.qkv_linear(rearrange(x, 'b l d -> b d l')), 'b d l -> b l d').chunk(chunks=3, dim=2)  # shapes: (b, l, d)
        ...
        x = rearrange(self.proj(rearrange(x, 'b l d -> b d l')), 'b d l -> b l d')  # (b, l, d)
