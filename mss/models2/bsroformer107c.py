...
class BSRoformer107c(nn.Module):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
        ...
        **kwargs
        ...
        # Transformer blocks
        n_hid = 6
        self.patch1 = FreqPatch(2, n_hid, (1, 1))
        self.enc1 = FreqBlock(dim=n_hid, n_layers=1)

        self.patch2 = Patch(n_hid*32, 96, (1, 1))
        ...
        self.unpatch2 = UnPatch(96, n_hid*32, (1, 1))

        self.cat1 = Cat(384, n_hid*32)
        self.dec1 = FreqBlock(dim=n_hid, n_layers=1)
        self.unpatch1 = FreqUnPatch(n_hid, 2, (1, 1))
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        x1 = self.patch1(x0)
        x1 = self.enc1(x1)
        x1 = rearrange(x1, '(b c) d k t f -> b (c d f) t k', b=B)
        ...
        y0 = self.cat1(x1, y1)
        y0 = rearrange(y0, 'b (c d f) t k -> (b c) d k t f', c=2, f=F_)
        y0 = self.dec1(y0)
        y0 = self.unpatch1(y0)

        # Postprocess
        x = y0[:, :, :, 0 : T, :]
        x = rearrange(x, '(b c) x k t f -> b c k t f x', b=B)
...
class FreqBlock(nn.Module):
    def __init__(self, dim, n_layers):
        ...
        self.rnn = nn.RNN(dim, dim // 2, num_layers=n_layers, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        y, _ = self.rnn(x)
        out = x + y
        out = rearrange(out, '(b k t) f d -> b d k t f', b=B, k=K)  # shape: (b, d, t, f)
