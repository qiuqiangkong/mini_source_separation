...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        q, k, v = self.qkv_linear(x).chunk(chunks=3, dim=3)  # shapes: (b, l, d)

        q = rearrange(self.norm_q(q), 'b t f (n h) -> b t f n h', h=self.head_dim)  # (b, l, n, h)
        k = rearrange(self.norm_k(k), 'b t f (n h) -> b t f n h', h=self.head_dim)  # (b, l, n, h)
        v = rearrange(v, 'b t f (n h) -> b t f n h', h=self.head_dim)  # (b, l, n, h)

        x1 = rearrange(F.scaled_dot_product_attention(
            query=rearrange(rope(rearrange(q, 'b t f n h -> (b f) t n h')), 'b l n h -> b n l h'), 
            key=rearrange(rope(rearrange(k, 'b t f n h -> (b f) t n h')), 'b l n h -> b n l h'), 
            value=rearrange(rearrange(v, 'b t f n h -> (b f) t n h'), 'b l n h -> b n l h'), 
            attn_mask=None, 
            dropout_p=0.0
        ), '(b f) n t h -> b t f (n h)', f=F_)

        x2 = rearrange(F.scaled_dot_product_attention(
            query=rearrange(rope(rearrange(q, 'b t f n h -> (b t) f n h')), 'b l n h -> b n l h'), 
            key=rearrange(rope(rearrange(k, 'b t f n h -> (b t) f n h')), 'b l n h -> b n l h'), 
            value=rearrange(rearrange(v, 'b t f n h -> (b t) f n h'), 'b l n h -> b n l h'), 
            attn_mask=None, 
            dropout_p=0.0
        ), '(b t) n f h -> b t f (n h)', t=T)
        
        x = self.proj(torch.cat([x1, x2], dim=-1))  # (b, l, d)
