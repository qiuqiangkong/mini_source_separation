...
from mss.models2.dsp3.banks import mel_linear_banks_triangle, erb_linear_banks_triangle, linear_banks_triangle
...
class BSRoformer115i(nn.Module):
    def __init__(
        self,
        audio_channels=2,
        sample_rate=48000,
        ...
        **kwargs
        ...
        # Subband filter
        banks1 = erb_linear_banks_triangle(sr=sample_rate, n_bands=111, max_half_bandwidth=max_half_bandwidth)
        self.sb_filter1 = SubbandFilter(sample_rate, banks1, factor, chunk_size=chunk_size)

        banks2 = mel_linear_banks_triangle(sample_rate, 118, max_half_bandwidth)
        self.sb_filter2 = SubbandFilter(sample_rate, banks2, factor, chunk_size=chunk_size)

        banks3 = linear_banks_triangle(sample_rate, 128)
        self.sb_filter3 = SubbandFilter(sample_rate, banks3, factor, chunk_size=chunk_size)

        # Patch
        in_channels = 3 * audio_channels * self.n_fft * 2
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        # Subband Analysis
        x1 = self.sb_filter1.analysis(audio)  # (b, c, k, l')
        x2 = self.sb_filter2.analysis(audio)  # (b, c, k, l')
        x3 = self.sb_filter3.analysis(audio)  # (b, c, k, l')
        ...
        x1 = self.sb_filter1.synthesis(x1)
        x2 = self.sb_filter2.synthesis(x2)
        x3 = self.sb_filter3.synthesis(x3)
